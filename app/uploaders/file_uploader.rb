class FileUploader < Shrine
  ...
  # Change a default derivative size by changing the config in your host app's application.rb file
  THUMB_SIZE = Forest.image_derivative_thumb_size
  SMALL_SIZE = Forest.image_derivative_small_size
  MEDIUM_SIZE = Forest.image_derivative_medium_size
  LARGE_SIZE = Forest.image_derivative_large_size
  IMAGE_DERIVATIVES = {
    thumb: {
      resize: { width: THUMB_SIZE,  height: THUMB_SIZE, fit: 'cover', withoutEnlargement: true }
    }.reverse_merge(DEFAULT_EDITS),
    small: {
      resize: { width: SMALL_SIZE,  height: SMALL_SIZE, fit: 'inside', withoutEnlargement: true }
    }.reverse_merge(DEFAULT_EDITS),
    medium: {
      resize: { width: MEDIUM_SIZE,  height: MEDIUM_SIZE, fit: 'inside', withoutEnlargement: true }
    }.reverse_merge(DEFAULT_EDITS),
    large: {
      resize: { width: LARGE_SIZE,  height: LARGE_SIZE, fit: 'inside', withoutEnlargement: true }
    }.reverse_merge(DEFAULT_EDITS)
  }
