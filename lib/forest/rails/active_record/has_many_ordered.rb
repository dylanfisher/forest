  ...
  module HasManyOrdered
    ...
    class_methods do
      def has_many_ordered(association, options = {})
        ...
        nested_attributes_options = options.delete(:nested_attributes_options) || {}
        ...
        accepts_nested_attributes_for through, **options.reverse_merge(allow_destroy: true, **nested_attributes_options)

        define_method("#{singular}_ids=") do |ids|
          ids = ids.reject(&:blank?).collect(&:to_i).uniq
          super(ids)
